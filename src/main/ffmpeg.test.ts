...
test('packaged Windows app resolves unpacked ffmpeg-static binary first',()=>{
  const resourcesPath=path.join('D:','Program Files','MultiPublish','resources');
  const expected=path.normalize(path.join(resourcesPath,'app.asar.unpacked','node_modules','ffmpeg-static','ffmpeg.exe'));
